...
contract L1Blocks is IL1Blocks {
    ...
    function getBlockTimestamp(uint256 blockNumber) external view override returns (uint256) { }

    function latestBaseFee() external view override returns (uint256) { }

    function getBaseFee(uint256 blockNumber) external view override returns (uint256) { }

    function latestBlobBaseFee() external view override returns (uint256) { }

    function getBlobBaseFee(uint256 blockNumber) external view override returns (uint256) { }

    function latestParentBeaconRoot() external view override returns (bytes32) { }

    function getParentBeaconRoot(uint256 blockNumber) external view override returns (bytes32) { }

    function setL1BlockHeader(bytes calldata blockHeaderRlp)
        external
        override
